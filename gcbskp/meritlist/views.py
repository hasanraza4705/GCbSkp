from django.shortcuts import render
from gcbskp.slider.models import Slider
from django.views.generic import TemplateView
from gcbskp.meritlist.models import MeritList
from gcbskp.offered_program.models import Programs
from django.core.paginator import Paginator
from gcbskp.admissionStatus.models import AdmissionStatus
from gcbskp.colgLogo.models import ColgLogo

def merit_list_view(request):
    merit_list_items = MeritList.objects.all().order_by('uploaded_at','id')
    offered_program_data = Programs.objects.all().order_by('id')
    admissionStatus = AdmissionStatus.objects.all()
    colg_logo = ColgLogo.objects.all()
    
    context = {
        'meritLists': merit_list_items,
        'ProgramData': offered_program_data,
        'admissionStatus' : admissionStatus,
        'colg_logo' : colg_logo
    }
    
    return render(request, "pages/merit_list.html", context)

def custom_admin_view(request):
    object_list = MeritList.objects.all().order_by('id')
    paginator = Paginator(object_list, 5)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    context = {
        'page_obj': page_obj,
    }
    return render(request, 'admin/custom_template.html', context)

